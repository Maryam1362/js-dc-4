...
// Hint: think about DOM methods that we can call on DOM elements we've already pulled from the page
 ...
 var galleryItem =gallery.querySelectorAll('.js-gallery-item')
 console.dir(gallery)
...
// We get the gallery and translate it to the left (so negative) by the width of one slide, every few seconds until the last one. 
// Then at the last one, we translate it back to the starting point (which is 0).

// To start, create a variable called slideCount that is equal to the number of slides and 
// another variable called slideWidth that is equal to the width of a single slide.

// To get the width, try getBoundingClientRect() or offsetWidth.


var number_slides= galleryItem.length
console.log(number_slides)

var offsetWidth =galleryItem[0].offsetWidth
...
// setInterval() takes two arguments: a reference to a function and the interval in milliseconds
//  between when setInterval should call that function.
...
function transitionSlide(){
	...
	if(currentslide < number_slides){
		console.log("te")
	
     console.log(gallery.style.transform = "translateX (-" + offsetWidth * currentslide + "px)")
